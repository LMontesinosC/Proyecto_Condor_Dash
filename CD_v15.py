...
class Servidor_local:
    ...
    def iniciar_juego_servidor(self):
        ...
        '''# 🔹 Hilo para manejar la lógica del juego
        hilo_juego = threading.Thread(target=self.juego_multijugador.juego_servidor, args=(True,), daemon=True)
        hilo_juego.start()
        print("🎮 [DEBUG] Hilo de juego del servidor iniciado.")

        # 🔹 Hilo para enviar datos a los clientes periódicamente
        hilo_envio_datos = threading.Thread(target=self._enviar_datos_periodicamente, daemon=True)
        hilo_envio_datos.start()
        print("📤 [DEBUG] Hilo de envío de datos iniciado.")

        for nombre, cliente in self.jugadores.items():
            if cliente:
                threading.Thread(target=self.recibir_actualizaciones_jugador, args=(nombre, cliente), daemon=True).start()'''
    ...
    def recibir_datos_servidor(self):
        ...
        try:
            while True:
                ...
                try:
                    datos_juego = self.juego_multijugador.desempaquetado_datos(data)
                    print(f"📩 [DEBUG] Datos procesados correctamente: {datos_juego}")  # 🔍 Verificar contenido

                    if isinstance(datos_juego, dict):
                        if "VACIO" in datos_juego.get("escenarios", []):
                        ...
                        print("⚠️ Datos de juego corruptos")

                except pickle.UnpicklingError:
                    print("❌ [ERROR] Error al desempaquetar datos: formato inválido.")
...
class Tuberias(Obstaculo):
    ...
    def render_basico(self, trayectoria, tipos):
        for i, (x, y) in enumerate(trayectoria):
            tipo, angulo = tipos[i] if i < len(tipos) else ("recto", 0)
            textura = pygame.transform.rotate(texturas[tipo], angulo)
            screen.blit(textura, (x + (ANCHO // 7.5), y))        
...
class Juego_multijugador:           # Servidor
    ...
    def juego_cliente(self, estado_juego):
        ...
        while estado_juego:
            ...
            print(f"{self.escenarios}") 
            if self.escenarios:
                print(f"{self.escenarios.obstaculos}") 
    ...
    def empaquetado_datos(self):
        ...
        # Evitar enviar un estado completamente vacío si antes hubo datos
        if not juego_data["escenarios"]:
            juego_data["escenarios"] = ["MANTENER"]  # En lugar de "VACIO", se mantiene el último escenario
        if not juego_data["objetos"]:
            juego_data["objetos"] = ["MANTENER"]
        if not juego_data["personajes"]:
            juego_data["personajes"] = {"MANTENER": {}}

        print(f"📦 [DEBUG] Datos empaquetados antes de enviar: {juego_data}")

        self.objetos_sv.clear()
        self.escenarios_sv.clear()
        return pickle.dumps(juego_data)
    ...
    def lectura_datos(self, datos_juego):
        try:
            print(f"📥 [DEBUG] Datos de juego recibidos: {datos_juego}")

            if not datos_juego:
                print("⚠️ [DEBUG] Datos de juego vacíos, esperando más...")
                return

            # Verificar si el escenario está en "MANTENER" y evitar sobrescribir
            if datos_juego.get("escenarios") and datos_juego["escenarios"][0] == "MANTENER":
                print("🔄 [DEBUG] Manteniendo escenario actual, no se cambia.")
            else:
                self.escenarios.append(self.crear_escenario(datos_juego["escenarios"][-1]))

            # Verificar si los objetos están en "MANTENER"
            if datos_juego.get("objetos") and datos_juego["objetos"][0] == "MANTENER":
                print("🔄 [DEBUG] Manteniendo objetos actuales.")
            else:
                self.escenarios[-1].obstaculos.append(self.crear_obstaculo(datos_juego["objetos"][-1]))

            # Verificar si los personajes están en "MANTENER"
            if datos_juego.get("personajes") and "MANTENER" in datos_juego["personajes"]:
                print("🔄 [DEBUG] Manteniendo estado de jugadores.")
            else:
                self.jugadores_cl = datos_juego["personajes"]
